"""VSCode backup service implementation."""
...
class VSCodeBackupService:
    """Service for backing up VSCode extensions."""
    ...
    def backup(self, config_dir: Path, output: Path | None = None) -> int:
        ...
        try:
            ...
            extensions = self._list_extensions(command)
            self._write_backup(output_file, extensions)
    ...
    def _list_extensions(self, command: str) -> list[str]:
        try:
    ...
    def _write_backup(self, destination: Path, extensions: list[str]) -> None:
