...
ssh-gk() {
	...
	ed25519 | rsa | ecdsa) ;;
	*)
		echo "Error: Unsupported key type '$type' (allowed: ed25519|rsa|ecdsa)." >&2
		return 1
		;;
	esac
	if ! [[ "$host" =~ ^[A-Za-z0-9._-]+$ ]]; then
		echo "Error: Invalid host '$host' (allowed: [A-Za-z0-9._-]+)." >&2
		return 1
	fi
	...
	if [ -f "$host_config_file" ]; then
		echo "Error: Config for host '$host' already exists." >&2
		return 1
	fi

	if [ -e "$keyfile_path" ] || [ -e "${keyfile_path}.pub" ]; then
		echo "Error: Key files already exist: '$keyfile_path'(.pub)." >&2
		return 1
	fi
...
ssh-ls() {
	find ~/.ssh/conf.d -name "*.conf" -type f 2>/dev/null | sed 's/\.conf$//' | awk -F/ '{print $NF}'
}
...
ssh-rm() {
	...
	local host_config_file="$HOME/.ssh/conf.d/${host}.conf"
	if [ ! -f "$host_config_file" ]; then
		echo "Error: Config for host '$host' not found." >&2
		return 1
	fi

	local keyfile
	keyfile=$(awk '/IdentityFile/ {print $2}' "$host_config_file")
	if [ -n "$keyfile" ]; then
