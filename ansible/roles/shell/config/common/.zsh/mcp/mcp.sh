...
_mcp_server_exists_global() {
	local mcp_name="$1"
	local server_exists
	server_exists=$(jq -r ".mcpServers | has(\"$mcp_name\")" ~/.mcp.json 2>/dev/null)
	[ "$server_exists" = "true" ]
}
...
mcp-ls() {
	...
	local mcp_names
	mcp_names=$(jq -r '.mcpServers | keys[]' ~/.mcp.json 2>/dev/null)
	...
	echo "$mcp_names" | while read -r mcp_name; do
		local command
		command=$(jq -r ".mcpServers[\"$mcp_name\"].command" ~/.mcp.json 2>/dev/null)
		local args
		args=$(jq -r ".mcpServers[\"$mcp_name\"].args[]?" ~/.mcp.json 2>/dev/null | tr '\n' ' ')
		...
		if [ "$command" != "null" ]; then
			...
			local description
			description=$(jq -r ".mcpServers[\"$mcp_name\"].description // \"No description available\"" ~/.mcp.json 2>/dev/null)
			echo "- $description"
			echo ""
		else
...
mcp-a() {
	...
	local server_config
	server_config=$(jq -r ".mcpServers[\"$mcp_name\"]" ~/.mcp.json 2>/dev/null)
