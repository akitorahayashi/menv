...
jq -r '.commands | to_entries[] | @base64' "$CONFIG_FILE" | while read -r row; do
    ...
    if [[ -f "ansible/roles/slash/config/common/$prompt_file" ]]; then
        cat "ansible/roles/slash/config/common/$prompt_file" >> "$output_file"
    else
        echo "Error: Prompt file not found: ansible/roles/slash/config/common/$prompt_file"
        exit 1
    fi
