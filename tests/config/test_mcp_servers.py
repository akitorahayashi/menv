    ...
    with servers_json_path.open("r") as f:
        try:
            ...
            raise AssertionError(
                f"Invalid JSON syntax in {servers_json_path}: {e}"
            ) from e
    return data
...
class TestMcpServers:
    def test_mcp_servers_structure(self, servers_data: dict) -> None:
        ...
        assert (
            "mcpServers" in servers_data
        ), "Missing 'mcpServers' key in the root object."
        assert isinstance(
            servers_data["mcpServers"], dict
    ...
    def test_mcp_server_definitions(self, servers_data: dict) -> None:
        ...
        for server_name, server_config in servers.items():
            assert isinstance(
                server_config, dict
            ), f"Server '{server_name}' config should be an object."

            for field, field_type in required_fields.items():
                assert (
                    field in server_config
                ), f"Server '{server_name}' is missing required field '{field}'."
                assert isinstance(
                    server_config[field],
                    field_type,
                ), (
            ...
            for arg in server_config["args"]:
                assert isinstance(
                    arg, str
                ), f"All items in 'args' for server '{server_name}' should be strings."

            # If the 'env' field exists, check that it is an object.
            if "env" in server_config:
                assert isinstance(
                    server_config["env"], dict
                ), f"Server '{server_name}' field 'env' should be an object."
