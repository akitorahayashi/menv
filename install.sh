...
main() {
    ...
    setup_ruby_env         # rbenvを使ったRuby環境のセットアップ
...
setup_ruby_env() {
    ...
    # zshrcかzprofileにrbenv初期化コードを追加（まだ追加されていない場合）
    SHELL_CONFIG="$HOME/.zprofile"
    if [ ! -f "$SHELL_CONFIG" ] || ! grep -q "rbenv init" "$SHELL_CONFIG"; then
        log_info "シェル設定ファイルにrbenvの初期化コードを追加します..."
        echo '# rbenv設定' >> "$SHELL_CONFIG"
        echo 'if command -v rbenv 1>/dev/null 2>&1; then' >> "$SHELL_CONFIG"
        echo '  eval "$(rbenv init -)"' >> "$SHELL_CONFIG"
        echo 'fi' >> "$SHELL_CONFIG"
        log_success "rbenvの初期化コードを追加しました"
    else
        log_success "rbenvの初期化コードはすでに設定されています"
    fi
