...
10. **Node.js Runtime & Tools (`nodejs` role)**
    ...
    -   **Cursor Integration**: Installs Cursor editor, downloads CLI, symlinks configuration files, and installs extensions when enabled (`--tags nodejs-cursor`).
    -   Conditional installation: Each component can be installed independently using tags (`--tags nodejs-platform`, `--tags nodejs-tools`, `--tags nodejs-claude`, `--tags nodejs-gemini`, `--tags nodejs-cursor`).

11. **MCP Servers Configuration (`mcp` role)**
