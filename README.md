...
6.  **macOS System Settings (`system` role)**
    ...
    -   A backup of the current system settings can be generated by running `make system-backup`. This uses the Python-driven `ansible/roles/system/utils/backup-system.py` script, which reads YAML definitions, executes `defaults read`, and renders the resulting configuration to `system.yml` without relying on external tools like `yq`.
